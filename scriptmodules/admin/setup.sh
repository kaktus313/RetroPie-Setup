...
function download_psx_images_setup()
{
    ...
    local psx_images_retropie_path="/home/pi/RetroPie/roms/psx"

    while true; do
        cmd=(dialog --backtitle "$__backtitle" --title "Downloading PSX images" --cancel-label "Exit" --no-tags --menu "Choose PSX image" 22 76 16 --file "/home/pi/psx_images.txt")

        choice=$("${cmd[@]}" 2>&1 >/dev/tty)
        [[ -z "$choice" ]] && break

        download_resource="https://archive.org/download/Sony-Playstation-EUR-Redump.org/"

        wget ${download_resource}"${choice:1}" -O $psx_images_retropie_path/temp.zip
        unzip $psx_images_retropie_path/temp.zip -d $psx_images_retropie_path
        rm $psx_images_retropie_path/temp.zip
